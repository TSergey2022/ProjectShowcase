...
import { fetchProjects, fetchTracks } from "../fetch";
...
function TrackPage() {
  ...
  useEffect(() => {
    fetchTracks().then((tracksData: {id: number, name: string}[]) => setTrackData(tracksData.find((track)=>track.id.toString()===id)))
  }, [id]);

  useEffect(() => {
    fetchProjects().then((newProjectsData) => setProjectsData(newProjectsData.filter((project: any)=>project.track.toString()===id)));
  }, [id, trackData]);
